...
const CompleteTaskCard = ({ task }) => {
  const {
    ...
    const handleNotComplete = () => {
        axios.patch(`http://localhost:5000/complete?id=${_id}`)
            .then(data => {
                console.log(data);
            })
            .catch(err => {
        ...
        <div className="block max-w-sm h-56 p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          ...
          <div className="flex mt-3 gap-4 justify-end">
            ...
            <Tooltip content="Delete" placement="top" animation="duration-500">
              <button onClick={handleDelete} className="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg px-3 py-1 text-2xl text-center">
